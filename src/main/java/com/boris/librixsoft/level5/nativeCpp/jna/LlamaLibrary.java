...
public interface LlamaLibrary extends Library {
    ...
    class llama_chat_message extends Structure {
        public String role;
        public String content;
        public static class ByReference extends llama_chat_message implements Structure.ByReference {}
    }
    ...
    int llama_model_meta_val_str(Pointer model, String key, byte[] buf, long buf_size);
    ...
    boolean llama_vocab_is_eog(Pointer vocab, int token);

    // ─── Chat Template ─────────────────────────────────────────────────────
    // tmpl = null  ⟹ use model's embedded template (tokenizer.chat_template)
    // add_ass = true  ⟹ append the assistant generation turn at the end
    // Returns number of bytes written, or negative if buffer is too small
    int llama_chat_apply_template(String tmpl,
                                  llama_chat_message[] msgs, long n_msgs,
                                  boolean add_ass, byte[] buf, int length);
