...
class SettingsManagerTest {
    ...
    @Test
    void loadSettings_deserializesMultiAgent_yes() throws Exception {
        Path settingsFile = tempDir.resolve("multiagent.json");
        Files.writeString(settingsFile, """
                {
                  "model": {"baseUrl": "http://localhost:11434", "name": "test"},
                  "multi-agent": "yes"
                }
                """);

        Settings s = settingsManager.loadSettings(settingsFile.toString());
        assertNotNull(s);
        assertTrue(s.isMultiAgentEnabled());
    }

    @Test
    void loadSettings_deserializesMultiAgent_no() throws Exception {
        Path settingsFile = tempDir.resolve("multiagent_no.json");
        Files.writeString(settingsFile, """
                {
                  "model": {"baseUrl": "http://localhost:11434", "name": "test"},
                  "multi-agent": "no"
        ...
        Settings s = settingsManager.loadSettings(settingsFile.toString());
        assertNotNull(s);
        assertFalse(s.isMultiAgentEnabled());
    }

    @Test
    void loadSettings_multiAgentMissing_returnsFalse() throws Exception {
        Path settingsFile = tempDir.resolve("no_multiagent.json");
        Files.writeString(settingsFile, """
                {
                  "model": {"baseUrl": "http://localhost:11434", "name": "test"}
                }
    ...
    @Test
    void ensureExists_defaultsContainMultiAgent() throws Exception {
        Path settingsFile = tempDir.resolve("defaults_check.json");
        settingsManager.ensureExists(settingsFile.toString());

        String content = Files.readString(settingsFile);
        assertTrue(content.contains("multi-agent"));
    }
}
