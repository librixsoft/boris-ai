  ...
  setup() {
    ...
    const fetchGpuMemoryInfo = async () => {
      try {
        ...
        if (response.ok) {
          gpuMemoryInfo.value = await response.json()
        }
      } catch (error) {
      ...
      sendMessage()
       ...
       if (loadedModels.value.length > 0) {
        ...
        try {
          ...
          await refreshModels()
          await fetchGpuMemoryInfo() // Update GPU memory info immediately after loading
          selectedModel.value = result.id || resolvedId
    ...
    const saveSettings = async () => {
     ...
     const checkLlamaServerStatus = async () => {
       try {
         ...
         if (response.ok) {
           const data = await response.json()
           llamaServerInstalled.value = data.installed === true
         }
       } catch (error) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.status === 'success') {
            ...
            // Refresh page to update all statuses
    ...
    const sendMessage = async () => {
      ...
      try {
        ...
        const response = await api('/boris/v1/chat/completions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        addDebugLog('request', `POST /boris/v1/chat/completions`, { instruction: text })
        ...
        const result = await response.json()
        addDebugLog('response', `Result`, result)
        if (result.status === 'error') {
          throw new Error(result.message || 'Execution failed')
        }

         // Display result
         messages.value.push({ role: 'assistant', content: result.result || 'Command executed successfully', inputTokens: result.inputTokens || 0, outputTokens: result.outputTokens || 0, contextSize: result.contextSize || 0 })
         currentBotMessage.value = ''
         isAgentTask.value = false
    ...
    onMounted(async () => {
      ...
      // If a model is loaded but warmup not finished, start polling
      if (!warmupComplete.value && loadedModels.value.length > 0) {
        startWarmupPolling()
      }
